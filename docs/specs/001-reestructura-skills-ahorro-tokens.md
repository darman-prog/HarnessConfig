...
### Skill de comunicación asertiva (2026-09-23)
- Motivo: el usuario reportaba respuestas gigantes (6+ párrafos) y texto difícil de escanear. La regla ya existía en `AGENTS.md` (veredicto + máx 5 bullets) y en `uso-eficiente` §4, pero como *límite* sin doctrina de redacción, densidad ni diagramas — y en dos fuentes.
- Cambios: nueva skill `comunicacion-asertiva` (núcleo ~26 líneas + `references/DOCTRINA.md` bajo demanda), declarada **obligatoria en "Toda tarea"** junto a `uso-eficiente`; `AGENTS.md` conserva los límites duros y nombra la doctrina + las dos reglas de fallback ("en preguntas puras responde sin etiquetas", "respeta modo detallado"); `uso-eficiente` §4 pasa a puntero (fuente única).
- Cuatro decisiones de diseño: (1) **modo respuesta** para preguntas puras (primera línea = respuesta directa, sin etiquetas de estado); (2) **override** `modo detallado` / `modo resumen` respetado en el mismo turno y nunca auto-concedido; (3) los 2 probes nuevos son **respaldo del modo respuesta** (chat de pregunta, `opencode run`, subagentes), no del modo tarea (cubierto por la fila obligatoria + check bidireccional); (4) **precedencia**: si la skill contradice a `AGENTS.md`, gana `AGENTS.md`.
- Coste: description ~30 palabras (**+45 tok fijos/sesión**) + núcleo ~26 líneas (**~+310 tok por tarea**, al ser obligatoria) ≈ +2,8% de una sesión de 12,5k; `DOCTRINA.md` se lee solo bajo demanda (0 hasta entonces). Suma con lo anterior: +73 tok fijos por sesión desde el 2026-09-21.
- Verificación: guardarraíl verde (skills=34) + 3 checks fail-closed nuevos (puntero en `AGENTS.md`, límite duro "Maximo 5 bullets" **solo** en `AGENTS.md`, la skill declara la precedencia) demostrados con fixture (exit 1 en los 3) + 2 probes de modo respuesta. El check SSOT ya mordió en la primera pasada: el núcleo repetía "Maximo 5 bullets" y se corrigió para que el número viva solo en el núcleo inyectado.
- No verificado (fuera de alcance por coste): un assert automático del formato de salida vía `opencode run` (1 corrida ≈ 12,5k tokens). Verificación actual = estática + humo manual en TUI.
- Fail-closed añadido en el mismo lote: el parser exige el `---` de cierre del frontmatter (antes un `SKILL.md` sin cierre podía pasar si tenía `name:`/`description:`) y `sync-global.ps1` hace `throw` si el guardarraíl no existe (antes solo avisaba y sincronizaba a ciegas, contradiciendo el criterio de esta spec).
