...
2. **Copiar config del harness** (ver `references/AGENTS-MERGE.md` para el comando exacto):
   - `robocopy` de `.opencode/` template → destino, excluyendo `node_modules`.
   - `Copy-Item` de `opencode.json` template → destino.
3. **Generar docs**:
   - `PRODUCT.md` y `DESIGN.md` desde templates en `references/` con las respuestas.
   - `AGENTS.md`: merge por secciones (Stack/Comandos se actualizan con lo descubierto; resto se preserva). Si no existe, crea nuevo.
...
- Si el proyecto ya tiene `.opencode/`, el overwrite lo reemplaza (avisa antes de ejecutar).
- No creas sub-agentes nuevos; usa los existentes del harness.
- No modifiques `impecable/SKILL.md` ni `impeccable/SKILL.md`.
- No corras `/init`; el merge de AGENTS.md lo reemplaza.
- No copies `node_modules` (usa `robocopy ... /XD node_modules`).
- No copies `.git` del template.
